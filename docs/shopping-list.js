...
var myApp = angular.module('myApp', [])
  .factory('localStorage', [ '$http', '$q', function ($http, $q) {
    // SQL requests for creating tables if they are not exists yet
    var elemLists = [], objData = {};
    ...
    var selectDeviceCodeLocalDb = "SELECT device_code as code, sync FROM device",
      ...
      selectLaminations = "SELECT id, name FROM lamination_colors ORDER BY id",
      ...
      calculationPrice: function (construction, callback) {
        ...
        function next_8(result) {
          if(result.rows.length){
            ...
            if(construction.glassSquare.length) {
              for (var i = 0; i < construction.glassSquare.length; i++) {
                //console.log('('+construction.glassSquare[i]+'*'+priceObj.glassIds.price.price+')');
                priceTmp += construction.glassSquare[i] * priceObj.glassIds.price.price;
              }
  ...
  .controller('ShoppingListController', ['$scope', '$http', '$log', 'localStorage', function ($scope, $http, $log, localStorage) {

    var db = openDatabase('bauvoice', '1.0', 'bauvoice', 65536);


    /*localStorage.getAllWindowHardwares(function (result) {
      if(result.status){
        console.log(result);
      } else {
        console.log(result);
    ...
    var templ_1 = {
      ...
      "profileId":35,
      "glassId":145,
      "framesSize":[1300,1400,1300,1400],
      "sashsSize":[],
      "beadsSize":[1312,1212,1312,1212],
      "impostsSize":[],
      "shtulpsSize":[],
      "glassSize":[
        [1302,1202,1302,1202]
      ],
      "glassSquare":[1.565004],
      "frameSillSize":1300,
      "frameId":7,
      "frameSillId":8,
      "sashId":9,
    ...
    localStorage.calculationPrice(templ_1, function (result) {
      if(result.status){
    ...
    localStorage.getLastSync(function (result) {
      if(result.status){
        console.log(result);
      } else {
        console.log(result);
    ...
    localStorage.syncDb(function (result) {
      if(result.status){
        console.log(result);
      } else {
        console.log(result);
    ...
    localStorage.getAllProfileSystems(function (result) {
      if(result.status){
        console.log(result);
      } else {
        console.log(result);
    ...
    localStorage.getAllLaminations(function (result) {
      if(result.status){
        console.log(result);
      } else {
        console.log(result);
    ...
    localStorage.getUserInfo(function (result) {
      if(result.status){
        console.log(result);
      } else {
        console.log(result);
    ...
    localStorage.login({login : 'zavod', password : 'zavod'}, function (result) {
      if(result.status){
        console.log(result);
      } else {
        console.log(result);
    ...
    localStorage.clearDb(function (result) {
      if(result.status){
        console.log(result);
      } else {
        console.log(result);
