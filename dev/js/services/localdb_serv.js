    ...
    .factory(
      ...
      function (
          ...
          tablesLocalDB = {
            addition_folders: {
              ...
              prop:
                ...
                " max_size INTEGER",
              foreignKey:
                ", FOREIGN KEY(factory_id) REFERENCES factories(id), FOREIGN KEY(addition_type_id) REFERENCES addition_types(id)",
            },
            cities: {
              tableName: "cities",
              prop:
            ...
            price_koefficients: {
              ...
              prop:
                "element_id INTEGER," + "koef_id INTEGER," + "value NUMERIC",
              foreignKey: "",
            },
            countries: {
              tableName: "countries",
            ...
            lamination_factory_colors: {
              ...
              foreignKey:
                ", FOREIGN KEY(lamination_type_id) REFERENCES lamination_default_colors(id), FOREIGN KEY(factory_id) REFERENCES factories(id)",
            },
            lamination_types: {
            ...
            elements: {
              ...
              prop:
                ...
                " reg_coeff NUMERIC",
              foreignKey:
                ", FOREIGN KEY(factory_id) REFERENCES factories(id), FOREIGN KEY(glass_folder_id) REFERENCES glass_folders(id), FOREIGN KEY(margin_id) REFERENCES margin_types(id), FOREIGN KEY(supplier_id) REFERENCES suppliers(id), FOREIGN KEY(currency_id) REFERENCES currencies(id), FOREIGN KEY(element_group_id) REFERENCES elements_groups(id)",
            },
            ...
            users: {
              ...
              prop:
                ...
                " code_sync VARCHAR",
              foreignKey:
                ", FOREIGN KEY(factory_id) REFERENCES factories(id), FOREIGN KEY(city_id) REFERENCES cities(id)",
            },
            users_discounts: {
            ...
            lists: {
              ...
              prop:
                ...
                " glass_color INTEGER",
              foreignKey:
                ", FOREIGN KEY(parent_element_id) REFERENCES elements(id), FOREIGN KEY(parent_element_id) REFERENCES elements(id), FOREIGN KEY(list_group_id) REFERENCES lists_groups(id), FOREIGN KEY(add_color_id) REFERENCES addition_colors(id)",
            },
            list_contents: {
              ...
              foreignKey:
                ", FOREIGN KEY(parent_list_id) REFERENCES lists(id), FOREIGN KEY(rules_type_id) REFERENCES rules_types(id), FOREIGN KEY(direction_id) REFERENCES directions(id), FOREIGN KEY(lamination_type_id) REFERENCES lamination_types(id), FOREIGN KEY(window_hardware_color_id) REFERENCES window_hardware_colors(id)",
            },
            ...
            window_hardwares: {
              ...
              prop:
                "window_hardware_type_id INTEGER," +
                " min_width INTEGER," +
                " max_width INTEGER," +
                " min_height INTEGER," +
                " max_height INTEGER," +
        ...
        function updateLocalDB(tableName, elem, options) {
          ...
          let val = options[key];
          LocalDataBase[tableName] = LocalDataBase[tableName].filter(function (
            item
          ) {
            return item[key] !== val;
          });
        ...
        function deleteRowLocalDB(tableName, options) {
          ...
          if (LocalDataBase[tableName] && LocalDataBase[tableName].length) {
            LocalDataBase[tableName] = angular.copy(
              LocalDataBase[tableName].filter(function (item) {
                return item[key] !== val;
              })
            );
        ...
        function importUser(login, type) {
          ...
          $http
            .post(globalConstants.serverIP + query, {
              login: login,
            })
            .then(
              function (result) {
            ...
            .then(
              function (result) {
                if (result.data.status) {
                  ...
                  db.setItem("location", LocalLocationBase)
                    .then((value) => {
                      // Do other things once the value has been saved.
                      defer.resolve(value);
                    })
        ...
        function importAllDB(login, access) {
          ...
          $http
            ...
            .then(
              ...
              function (err) {
                console.log(
                  "Something went wrong with importing Database!",
                  err
                );
                defer.resolve(0);
              }
                    ...
                    if (index === arr.length - 1) {
                      if (angular.isArray(item2)) {
                        var promisHW = _.map(item2, function (item3) {
                          var deff3 = $q.defer();
                          parseListContent(item3.child_id).then(function (
                            result4
                          ) {
                            if (result4.length) {
              ...
              if (item) {
                if (angular.isArray(item)) {
                      ...
                      if (angular.isArray(item2)) {
                        var promisHW = _.map(item2, function (item3) {
                          ...
                          if (item3.child_type === "element") {
                            deff3.resolve(
                              getElementByListId(1, item3.child_id)
                            );
                          } else {
                  ...
                  if (index === arr.length - 1) {
                    ...
                    deff1.resolve(
                      getElementByListId(0, item.parent_element_id)
                    );
                  }
                }
              } else {
                    ...
                    var promConsistElem2 = _.map(item2, function (item3) {
                      ...
                      if (item3) {
                        if (angular.isArray(item3)) {
                          var promConsistElem3 = _.map(item3, function (item4) {
                            ...
                            if (item4) {
                              if (item4.child_type === "element") {
                                ...
                                getKitByID(item4.child_id).then(function (
                                  data4
                                ) {
                          ...
                          if (item3.child_type === "element") {
                            deff3.resolve(
                              getElementByListId(0, item3.child_id)
                            );
                          } else {
                            getKitByID(item3.child_id).then(function (data) {
                              angular.extend(item3, data);
            ...
            for (block = 0; block < sizeQty; block += 1) {
              ...
              if (sizes[block].elemId === kits.id) {
                ...
                while (--sizeBeadQty > -1) {
                  ...
                  constrElem.priceReal = GeneralServ.roundingValue(
                    priceTemp,
                    3
                  );
                ...
                if (ProductStor.product.door_type_index !== 0) {
                  ...
                  if (
                    ...
                    if (ProductStor.product.door_type_index !== 0) {
                      ...
                      if (
                        ProductStor.product.door_type_index === 2 ||
                        ProductStor.product.door_type_index === 3
                      ) {
                        sizeTemp =
                          (ProductStor.product.template_height -
                            ProductStor.product.profileDepths.frameDepth.c -
        ...
        function culcPriceAsRule(
          ...
          if (currConsistElem) {
            ...
            if (
              currConsist.rounding_type > 0 &&
              currConsist.rounding_value > 0
            ) {
            ...
            if (sizeReal) {
              ...
              priceReal =
                sizeReal * qtyReal * currConsistElem.price * wasteValue;
            } else {
                        ...
                        for (var el = 0; el < hwElemQty2; el += 1) {
                          if (
                            ...
                            if (
                              ...
                              currConsist[hwInd][hwInd2].newValue =
                                angular.copy(objTmp.qty);
                            }
                          }
                        }
                      }
        ...
        function calculationPrice(construction) {
          ...
          if (
            ...
            if (
              ProductStor.product.profile.id === 345 ||
              ProductStor.product.profile.id === 527 ||
              ProductStor.product.profile.id === 541 ||
              ProductStor.product.profile.id === 547 ||
              ProductStor.product.profile.id === 571
            ) {
              temp_profile_id = 416611;
            }
            if (
              ProductStor.product.profile.id === 25 ||
            ...
            if (temp_profile_id) {
              if (ProductStor.product.template_square >= 1) {
                selectLocalDB(tablesLocalDB.elements.tableName, {
                  id: temp_profile_id,
                }).then(function (result) {
                  if (result && result.length) {
                    ...
                    tmp[0].qty = 1;
                    if (
                      ProductStor.product.template_square >= 1 &&
                      ProductStor.product.template_square < 2
                    ) {
                      tmp[0].qty = 0.5;
                    }
                    if (
                      ProductStor.product.template_square >= 2 &&
                      ProductStor.product.template_square < 3
                    ) {
                      tmp[0].qty = 0.75;
                    }
                    if (
                      ProductStor.product.template_square >= 3 &&
                      ProductStor.product.template_square < 4
                    ) {
                      tmp[0].qty = 1;
                    }
                    if (
                      ProductStor.product.template_square >= 4 &&
                      ProductStor.product.template_square < 5
                    ) {
                      tmp[0].qty = 1.25;
                    }
                    ...
                    tmp[0].sizeLabel = 0;
                    tmp[0].priceReal =
                      currencyExgange(tmp[0].price, tmp[0].currency_id) *
                      tmp[0].qty;
        ...
        function calcDoorElemPrice(handleSource, lockSource) {
          ...
          getElementByListId(0, handleSource.parent_element_id).then(function (
            handleData
          ) {
            ...
            (function nextRecord() {
              if (list.length) {
                ...
                selectLocalDB(tablesLocalDB.lists.tableName, {
                  ...
                  parseListContent(firstKitId.parent_element_id).then(function (
                    result2
                  ) {
                    if (result2 !== 0) {
                            ...
                            if (element.child_type === "list") {
                              ...
                              getElementByListId(
                                ...
                                if (firstKitId.count) {
                                  if (element.value) {
                                    kit.value =
                                      firstKitId.count * element.value;
                                  } else {
                                  ...
                                  if (element.value) {
                                    kit.value =
                                      firstKitId.value * element.value;
                                  } else {
                ...
                parseConsistElem([priceObj.consist]).then(function (
                  consistElem
                ) {
        ...
        function getAdditionalPrice(AddElement) {
          ...
          /** collect Kit Children Elements*/
          parseListContent(angular.copy(AddElement.elementId)).then(function (
            result
          ) {
            ...
            getKitByID(AddElement.elementId).then(function (kits) {
              if (kits) {
                ...
                /** parse Kit Element */
                getElementByListId(0, priceObj.kits.parent_element_id).then(
                  function (kitsElem) {
                    ...
                    parseConsistElem([priceObj.consist]).then(function (
                                  ...
                                  for (var el = 0; el < consistQty; el++) {
                                    if (
                                      ...
                                      wasteValue = priceObj.consist[el].waste
                                        ...
                                        : 1;
                                      if (
                                        priceObj.consist[cons].child_type ===
                                        "list"
                                      ) {
                                        priceObj.consist[cons].newValue =
                                          getValueByRule(
                      ...
                      finishPriceObj.priceTotal = angular.copy(
                        priceObj.priceTotal
                      );
                      deffMain.resolve(finishPriceObj);
                    });
        ...
        function calculationGridPrice(AddElement) {
          ...
          /** parse Kit */
          $q.all([getKitByID(grid.cloth_id), getKitByID(grid.top_id)]).then(
            function (kits) {
          ...
          for (let index = 0; index < key_length; index++) {
            ...
            if (elem_koef_number !== 0) {
              ...
              if (keys[index] === "elements") {
                let price_koefficients_length =
                  input.tables.price_koefficients.rows.length;
                for (
                  let index = 0;
                  index < price_koefficients_length;
                  index++
                ) {
              ...
              for (let kndex = 0; kndex < curr_row_length; kndex++) {
                if (curr_table.fields[kndex] === "img" && curr_row[kndex]) {
                  ...
                  if (is_cordova) {
                    ...
                    new_row[curr_table.fields[kndex]] =
                      globalConstants.serverIP + curr_row[kndex];
                  }
                } else {
                  if (curr_table.fields[kndex] !== "glass_id") {
                    new_row[curr_table.fields[kndex]] = checkStringToQuote(
                      curr_row[kndex]
                    );
                  } else {
                    new_row[curr_table.fields[kndex]] = curr_row[kndex];
