  ...
  angular
    ...
    .factory("localDB", function(
      ...
      var thisFactory = this,
        ...
        tablesLocalDB = {
          ...
          currencies: {
            ...
            prop:
              "name VARCHAR(100), value NUMERIC(10, 2), factory_id INTEGER, is_base INTEGER",
          ...
          beed_profile_systems: {
            ...
            prop:
              "profile_system_id INTEGER, list_id INTEGER, glass_width INTEGER",
          ...
          lamination_factory_colors: {
            ...
            prop:
              "name VARCHAR(255), lamination_type_id INTEGER, factory_id INTEGER",
            foreignKey:
          ...
          rules_types: {
            ...
            prop:
              "name VARCHAR(255), parent_unit INTEGER, child_unit INTEGER, suffix VARCHAR(15)",
            foreignKey: ""
          },
          regions: {
            tableName: "regions",
            prop:
              "name VARCHAR(255), country_id INTEGER, heat_transfer NUMERIC(10, 2), climatic_zone NUMERIC",
          ...
          window_hardware_handles: {
            ...
            prop:
              "element_id INTEGER, location VARCHAR(255), constant_value NUMERIC(10, 2)",
            foreignKey: ""
          },
            ...
            db.transaction(function(trans) {
              trans.executeSql(
                "CREATE TABLE IF NOT EXISTS " +
                  tablesLocalDB[table].tableName +
                  " (id INTEGER PRIMARY KEY AUTOINCREMENT, " +
                  tablesLocalDB[table].prop +
                  ", modified TIMESTAMP DEFAULT CURRENT_TIMESTAMP" +
                  tablesLocalDB[table].foreignKey +
                  ")",
                [],
                function() {
                  defer.resolve(1);
                ...
                function() {
                  console.log(
                    "Something went wrong with creating table " +
                      tablesLocalDB[table].tableName
                  );
                  defer.resolve(0);
                }
          ...
          for (t = 0; t < tableQty; t += 1) {
            ...
            if (rowsQty) {
              for (r = 0; r < rowsQty; r += 1) {
                var defer = $q.defer(),
                  values = _.map(result.tables[tableKeys[t]].rows[r], function(
                    elem
                  ) {
      ...
      function selectLocalDB(tableName, options, columns) {
        ...
        db.transaction(function(trans) {
          trans.executeSql(
            "SELECT " + properties + " FROM " + tableName + vhereOptions,
            [],
            function(tx, result) {
      ...
      function getLastSync(callback) {
        db.transaction(function(transaction) {
          transaction.executeSql(
            "SELECT last_sync FROM device",
            [],
            function(transaction, result) {
              if (result.rows.length) {
                callback(
                  new OkResult({ last_sync: result.rows.item(0).last_sync })
                );
              } else {
            ...
            function() {
              callback(
                new ErrorResult(
                  2,
                  "Something went wrong with selection last_sync record"
                )
              );
            }
          );
      ...
      function syncDb(login, access_token) {
        ...
        getLastSync(function(result) {
          ...
          $http
            .get(
              "http://api.voice-creator.net/sync/elements?login=" +
                login +
                "&access_token=" +
                access_token +
                "&last_sync=" +
            ...
            .success(function(result) {
              db.transaction(function(transaction) {
                if (result.tables.length) {
                  for (table in result.tables) {
                    for (i = 0; i < result.tables[table].rows.length; i += 1) {
                      ...
                      for (
                        k = 0;
                        k < result.tables[table].fields.length;
                        k += 1
                      ) {
                        if (!k) {
                          updateSql +=
                            result.tables[table].fields[k] +
                            " = '" +
                            result.tables[table].rows[i][k] +
                            "'";
                        } else {
                          updateSql +=
                            ", " +
                ...
                transaction.executeSql(
                  "UPDATE device SET sync = 1, last_sync = ? WHERE id = 1",
                  ["" + result.last_sync + ""],
                  function() {
                  ...
                  function() {
                    console.log(
                      "Something went wrong with updating device table!"
                    );
                  }
                );
              });
            ...
            if (item.length && construction.ids[index]) {
              ...
              if (index === arr.length - 1) {
                ...
                if (angular.isArray(construction.ids[index])) {
                  var promisKits = _.map(construction.ids[index], function(
                    item2
                  ) {
                    var deff2 = $q.defer();
                    selectLocalDB(
