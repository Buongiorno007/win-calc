...
//        console.log('normal = ',  normal);
      ...
      if(line1.to.dir === 'curv') {
        crossPoint.dir = 'curv';
        crossPoint.heightQP = line1.to.heightQP;
        crossPoint.positionQ = line1.to.positionQ;

        var curvChord = {
              from: angular.copy(line1.from),
              to: angular.copy(line2.to)
            };
        setLineCoef(curvChord);
        var coordQP = setQPointCoord(crossPoint.positionQ, curvChord, crossPoint.heightQP);
        crossPoint.xQ = coordQP.x;
        crossPoint.yQ = coordQP.y;
console.log('QP+++++++++', coordQP);
        //------ if curve vert or hor
        if (!line1.coefA && crossPoint.positionQ === 1) {
          crossPoint.yQ -= crossPoint.heightQP * 2;
        } else if (!line1.coefB && crossPoint.positionQ === 4) {
          crossPoint.xQ -= crossPoint.heightQP * 2;
        }
      ...
      if(qpQty) {
        for (var q = 0; q < qpQty; q++) {
          ...
          if (curvP0 && curvP1) {
            ...
            coordQP = setQPointCoord(curvQP.positionQ, curvLine, curvQP.heightQP);
            curvQP.xQ = coordQP.x;
            curvQP.yQ = coordQP.y;
...
//            if (pointsIn[pointsInQty].id.slice(0, 3).indexOf('qa') + 1 || pointsIn[pointsInQty].id.slice(0, 3).indexOf('qc') + 1) {
    ...
    function getCoordSideQPCurve(t, p0, p1) {
      ...
      var qpi = {
        x: Math.round( ((1-t)*xP0 + t*xP1) * 100)/100,
        y: Math.round( ((1-t)*yP0 + t*yP1) * 100)/100
      };
      return qpi;
    }
...
//      console.log('FINISH', dim);
    ...
    function setRadiusCoordXCurve(pointQ, P0, QP, P1) {
      pointQ.startX = getCoordCurveByT(P0.x, QP.xQ, P1.x, 0.5);
      pointQ.startY = getCoordCurveByT(P0.y, QP.yQ, P1.y, 0.5);
