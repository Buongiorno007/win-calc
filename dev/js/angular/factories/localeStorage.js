myApp.factory('localeStorage', function(){
    return {
        md5: function(string){
            ...
            db.transaction(function(transaction) {
                transaction.executeSql(selectUser, [], function(transaction, result) {
                    if(result.rows.length){
                        var resultObj = {username : result.rows.item(0).name, cityid : result.rows.item(0).city_id, cityname : result.rows.item(0).city_name, avatar : result.rows.item(0).avatar};
                        callback(new OkResult(resultObj));
                    } else {
