        ...
        db.transaction(function (trans) {
          trans.executeSql("SELECT * FROM " + tableName + vhereOptions, [],
            function (tx, result) {
              ...
              var resultKeys = Object.keys(result.rows),
                  resultQty = resultKeys.length;
              console.log(resultKeys, resultQty);
              if (resultQty) {
//                var resultARR = [];
//                for(var t in result.rows) {
//                  resultARR.push(result.rows[t]);
//                }

                var resultARR = resultKeys.map(function(prop) {
                  console.log(prop);
                  return result.rows.item(prop);
                });
...
/*
      importDb: function (login, access) {
...
//              var tables = {currencies: 1, list_contents: 1};
//              var tables = {currencies: 1};
//              for (var table in tables) {
                for (var i = 0; i < result.tables[table].rows.length; i++) {
              ...
              transaction.executeSql('INSERT INTO user (modified) VALUES (?)', [""+result.last_sync+""], function(){
