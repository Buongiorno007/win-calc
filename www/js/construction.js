    ...
    //------ CORNER IMPOST
      ...
      if(widthBlock > minLimitSize && heightBlock > minLimitSize) {
        ...
        for(var i = 0; i < currGlassPackage.parts.length; i ++) {
          var checkX = currGlassPackage.parts[i].toPoint.x;
          var checkY = currGlassPackage.parts[i].toPoint.y;

          var result = checkPoint(x1, y1, x2, y2, checkX, checkY);
        ...
        centerX = centerX/currGlassPackage.parts.length;
        centerY = centerY/currGlassPackage.parts.length;
        for(var i = 0; i < currGlassPackage.parts.length; i ++) {

          var fi = Math.atan2(centerY - currGlassPackage.parts[i].toPoint.y, currGlassPackage.parts[i].toPoint.x - centerX) * (180 / Math.PI);
          if(fi < 0) {
            fi = 360 + fi;
          }
          console.log('fi == ', fi);
...
*/
  ...
  function checkPoint(x1, y1, x2, y2, checkX, checkY) {
    return (x2-x1)*(y2-checkY)-(y2-y1)*(x2-checkX);
  }
